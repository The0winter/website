...
export async function readBusiness(db, request = {}, now = Date.now()) {
  ...
  const [totalUsers,registrations,unknownDates,reads,topBooks,bookmarks]=await Promise.all([
    ...
    query('users',[{$match:{created_at:{$type:'date',$gte:new Date(previousStart+'T00:00:00+08:00'),$lt:new Date(shift(1)+'T00:00:00+08:00')}}},{$group:{_id:{$dateToString:{date:'$created_at',format:'%Y-%m-%d',timezone:timeZone}},count:{$sum:1}}},{$sort:{_id:1}}]),
  ...
  return {sampledAt,days,timeZone,startDate,endDate,totalUsers,unknownRegistrationDates:unknownDates,todayNewUsers:reg.get(today)||0,
