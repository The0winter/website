...
const root=workspace(),directory=path.join(projectRoot,'.runtime/task-artifacts/site-monitor');
...
try {
  ...
  await page.locator('[data-page="server"]').click();await page.screenshot({path:path.join(directory,'final-live-server.png'),fullPage:true});
  await page.locator('[data-page="r2"]').click();await page.screenshot({path:path.join(directory,'final-live-r2.png'),fullPage:true});
  assert.deepEqual(errors,[]);
  const state=app.snapshot(),summary={verifiedAt:new Date().toISOString(),modules:Object.fromEntries(Object.entries(state.modules).map(([k,v])=>[k,{status:v.status,points:v.history.length}])),atlasBytes:state.modules.atlas.data.cluster?.logicalBytes,r2:state.inventory.buckets.map(({id,bytes,objects,complete})=>({id,bytes,objects,complete}))};
  await page.close();
