...
function visibleReport(report) {
  ...
  return Object.fromEntries(['title', 'author', 'jobId', 'downloaded', 'expected', 'errors', 'warnings', 'structuralPass', 'completeAgainstSource', 'exportFile', 'summaryFile', 'reportFile', 'limitation', 'reusedExport'].map(key => [key, report[key]]));
}
    ...
    try {
      ...
      if (pathname === '/api/search') {
        ...
        try {
          ...
          if (controller.signal.aborted) { stoppedTask(); return respond(200, {candidates: [], settings, task}); }
          update({phase: 'error', message: error.message}); throw error;
      ...
      if (pathname === '/api/start') {
        ...
        worker.on('message', message => {
          ...
          if (message.type === 'error') { if (stopRequested) stoppedTask(); else update({phase: 'error', message: message.error}); }
        });
