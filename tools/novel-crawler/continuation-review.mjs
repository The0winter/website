...
import {continuationKey, recordContinuationReview, recordContinuationAnchorReview, recordContinuationNoticeReview, recordContinuationPartPolicy, bindReviewedCompletedSource, recordContinuationNumberReset, recordContinuationNumberCorrection} from './continuation.mjs';
import {withLock} from './storage.mjs';

try {
  const {values} = parseArgs({options: {...Object.fromEntries(['spec', 'state-dir', 'output-dir', 'first', 'second', 'keep', 'reason', 'anchor-file', 'old-number', 'new-link', 'old-hash', 'new-hash', 'notice-link', 'chapter-parts', 'completed-source', 'number-reset', 'number-correction'].map(name => [name, {type: 'string'}])),
  ...
  const result = await withLock(path.join(stateDir, 'book-locks', continuationKey(spec) + '.lock'), () => values['number-correction']
