...
test('Mongo 实测：去重、跨会话保留、观察边界、原始证据过期和同源 CSRF',async t=>{
  ...
  const result=await readTraffic(db,{},Date.parse('2026-09-15T23:00:00+08:00'));
  assert.equal(result.views.raw.rolling[1].activeUsers,3);assert.equal(result.views.retained.rolling[1].activeUsers,2);assert.equal(result.views.retained.rolling[1].newUsers,2);assert.equal(result.views.raw.rolling[1].newUsers,2);assert.equal(result.views.raw.rolling[30].activeUsers,null);assert.equal(result.views.raw.trends.day[0].activeUsers,null);assert.equal(result.views.raw.trends.month.at(-1).activeUsers,null);assert.equal(result.mode,'observe');assert.equal(result.reviewReady,true);assert.ok(!JSON.stringify(result).includes(visitor));assert.ok(!JSON.stringify(result).includes('bot-only'));
  const nextDay=await readTraffic(db,{},Date.parse('2026-09-16T03:00:00Z'));assert.equal(nextDay.views.raw.rolling[1].activeUsers,0);assert.equal(nextDay.views.raw.rolling[7].activeUsers,3);assert.equal(nextDay.views.retained.rolling[7].activeUsers,2);assert.equal(nextDay.views.raw.trends.day.at(-1).activeUsers,3);assert.equal(nextDay.views.raw.rollingEndDate,'2026-09-16');
