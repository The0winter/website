...
const env={...process.env,APP_ENV:'test',DATABASE_URL:replica.getUri('test1_test'),JWT_SECRET:crypto.randomBytes(48).toString('hex'),EXTERNAL_SERVICES:'disabled',MAIL_MODE:'capture',RUN_JOBS:'enabled',PORT:'5006'};
...
await mongoose.connect(env.DATABASE_URL,{autoIndex:false,autoCreate:false,serverSelectionTimeoutMS:5000});
try{
  ...
  await fs.writeFile('.runtime/process-recovery-report.json',JSON.stringify(report,null,2));console.log(JSON.stringify(report));
