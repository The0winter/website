...
test('SMTP delivery uses loopback receiver, propagates failure and refuses external test targets',async()=>{
  const messages=[];let reject=false,stall=false;
  const receiver=net.createServer(socket=>{
    socket.setTimeout(5000,()=>socket.destroy());if(stall)return;socket.write('220 isolated ESMTP\r\n');
  ...
  try {
    ...
    stall=true;const started=Date.now();await assert.rejects(sendEmail('recipient@example.test','654321'),/Greeting never received/);assert.ok(Date.now()-started<4500);stall=false;
