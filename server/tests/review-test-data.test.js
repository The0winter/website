...
test('test review batches are visible, isolated from reader scores, idempotent and removable',async()=>{
  ...
  try{
    ...
    assert.equal((await request(`/api/books/${book.id}/reviews`,{rating:4,sourceExcerpt:{url:'https://example.org'}},'POST')).status,400);
    ...
    const renamed={...smaller,previousDrafts:smaller.drafts,drafts:smaller.drafts.map((row,index)=>({...row,displayName:`新昵称_${index+1}`}))};
    await User.updateOne({_id:user._id},{$set:{username:'占用昵称（测试）'}});
    const collision={...renamed,drafts:renamed.drafts.map((row,index)=>index===1?{...row,displayName:'占用昵称'}:row)};
    await assert.rejects(manageReviewTestData(collision,{mode:'replace',writeAudit}),/already in use/);
    assert.equal((await User.findById(sample.userId)).username,sample.username);
    assert.equal((await User.findById(user._id)).username,'占用昵称（测试）');
    await User.updateOne({_id:user._id},{$set:{username:user.username}});
    assert.equal((await manageReviewTestData(renamed,{mode:'replace',writeAudit})).changed,true);
    assert.equal((await manageReviewTestData(renamed,{mode:'replace',writeAudit})).changed,false);
    const renamedProfile=(await request(`/api/users/${sample.userId}/profile`)).body;
    assert.equal(renamedProfile.username,'新昵称_1（测试）');assert.equal(renamedProfile.isTestAccount,true);
    assert(!('email' in renamedProfile));
    assert.equal((await request(`/api/books/${book.id}/review-reactions?ids=${sample.reviewId}`)).body[0].likes,1);
    assert.equal((await request(`/api/books/${book.id}/reviews?limit=50`)).headers.get('x-rating-summary'),JSON.stringify(summary));
    assert(audit.some(row=>row.phase==='prepared'&&row.previousUsers?.some(old=>old.username===sample.username)));
    smaller=renamed;
