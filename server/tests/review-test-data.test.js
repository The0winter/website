...
test('test review batches are visible, isolated from reader scores, idempotent and removable',async()=>{
  ...
  try{
    ...
    await assert.rejects(manageReviewTestData(plan,{mode:'cleanup',writeAudit}),/changed/);
    assert.equal(await Review.countDocuments(),22);
    await Review.updateOne({_id:sample.reviewId},{$set:{content:sample.content}});
    assert.equal((await manageReviewTestData(plan,{mode:'cleanup',writeAudit})).changed,true);
    assert.equal((await manageReviewTestData(plan,{mode:'cleanup',writeAudit})).changed,false);
