...
for(const file of files){
 const code=await new Promise((resolve,reject)=>{const child=spawn(process.execPath,['infra/import-local.mjs',file,...(process.argv.includes('--apply')?['--apply']:[])],{stdio:'inherit',windowsHide:true});child.on('error',reject);child.on('exit',resolve);});
