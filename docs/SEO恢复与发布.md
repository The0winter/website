...
- 首页：`九天小说站 - 热门小说免费在线阅读 - 笔趣阁`，合并重复的描述词，保留原有站名及首页关键词。
- 书籍与目录共用详情页：`书名_作者_在线免费阅读 - 九天小说站`；没有作者时省略该段，不写虚构作者或空分隔符。
- 章节：`第N章 章节名 - 书名 - 九天小说站`，保留具体章节信息；直接访问与阅读器切章使用同一标题规则。
- 作者：`作者的小说作品 - 九天小说站`；翻页时保留页码。
- 排行榜、社区及讨论继续使用与页面用途相符的独立标题。账户等私有页面仍然 noindex。
- 书籍摘要先说明书名、作者、目录和免费阅读，再接作品简介；移除 HTML 标签及多余空白。标题、摘要与 Open Graph 分享信息保持一致。

标题由服务器输出。搜索结果可能自行改写标题与摘要，修改后需要搜索引擎重新抓取，不能据此承诺排名或立即收录。

首页在服务端输出一份 `WebSite` JSON-LD，站名与 Open Graph 共用 `siteName`，网址使用公开规范首页地址。页脚简介描述小说介绍、目录、阅读和找书功能，不再输出开发技术栈说明。站名标记用于明确网站身份，不代表排名或收录保证。

首页的 description、Open Graph、WebSite 描述与可见页脚简介共用 `siteDescription`；手机首页也展示完整简介。首页评分、热度、重复的无障碍书单及页脚导航等零散文字使用 `data-nosnippet`，供 Google、Bing 等支持此属性的搜索引擎排除摘要片段，书籍链接仍可抓取。百度未确认支持此属性；完整简介只是摘要候选，百度会结合搜索词自行选取，旧结果需要重新抓取后才可能更新。
