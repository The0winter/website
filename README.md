...
启动：`npm run dev:isolated`。自动启动真实 MongoDB 7.0.14 单节点副本集（首次需下载官方二进制），只监听回环，生成合成数据和临时密钥。退出后测试库销毁，不读取旧数据库配置。
...
本地默认关闭真实邮件、媒体、SEO、广告、统计；邮件暂由内存接收器捕获（不暴露公网）。测试库是真实 mongod，不是 Mongoose mock。单节点副本集支持事务但不是高可用部署。
