...
export default function MobileWriterDialog({ onClose }: { onClose: () => void }) {
  ...
  const navigate = useRef<(restoreCenter?: boolean) => void>(() => {});
  ...
  useEffect(() => {
    ...
    navigate.current = (restoreCenter = false) => {
      restoreOnReturn = restoreCenter;
      // Writer actions keep the center as their parent; login keeps its existing return behavior.
      if (!restoreCenter) clearHistoryMarker();
    };
    ...
    return () => {
      ...
      if (!restoreOnReturn) clearHistoryMarker();
  ...
  return createPortal(<dialog ref={dialog} className="mw-dialog" aria-labelledby="mw-title" onCancel={event => { event.preventDefault(); dismiss.current(); }}>
    ...
    <div className="mw-scroll" onAnimationEnd={event => { if (event.target === event.currentTarget && event.animationName === 'mw-content-in') finishOpening(); }}>
      ...
      {authLoading ? <p className="mw-status" role="status"><Loader2 className="animate-spin" size={19}/>正在确认登录状态…</p> : !user ? <section className="mw-guest"><FilePenLine size={32}/><h3>你的故事，值得被读到</h3><p>登录后创建作品、保存草稿，<br/>也可以接着写上次未完成的章节。</p><Link prefetchMode="intent" href="/login" className="mw-primary" onNavigate={() => navigate.current()}>登录并开始创作<ArrowRight size={17}/></Link></section> : <>
        <div className="mw-actions"><Link prefetchMode="intent" href="/writer?action=new&from=creation" className="mw-action mw-action-primary" onNavigate={() => navigate.current(true)}><Plus size={23}/><strong>新建作品</strong><span>开启一个新故事</span></Link><Link prefetchMode="intent" href="/writer?from=creation" className="mw-action" onNavigate={() => navigate.current(true)}><BookOpen size={23}/><strong>作品管理</strong><span>章节 · 草稿 · 设置</span></Link></div>
        <section className="mw-works" aria-label="我的作品"><div className="mw-section-heading"><h3>我的作品</h3><span>从上次落笔处继续</span></div>
          {loading ? <p className="mw-status" role="status"><Loader2 className="animate-spin" size={19}/>正在翻开你的作品…</p> : error ? <div className="mw-status" role="alert"><p>{error}</p><button type="button" onClick={() => setRetry(value => value + 1)}>重新加载</button></div> : books.length ? <div className="mw-book-list">{books.map(book => <article className="mw-book" key={book.id}><div className="mw-cover">{book.cover_image ? <BookCover src={book.cover_image} alt={`${book.title}封面`} sizes="64px"/> : <BookOpen size={26}/>}</div><div className="mw-book-info"><h4>{book.title}</h4><p>{book.category || '未分类'} · {['completed', '完结'].includes(book.status || '') ? '已完结' : '连载中'}</p><div><Link prefetchMode="intent" href={writerHref(book, 'write')} onNavigate={() => navigate.current(true)}><PenTool size={14}/>写一章</Link><Link prefetchMode="intent" href={writerHref(book, 'manage')} onNavigate={() => navigate.current(true)}>目录与草稿<ChevronRight size={14}/></Link></div></div></article>)}</div> : <div className="mw-empty"><FilePenLine size={34}/><h4>{page === 1 ? '第一部作品，从这里开始' : '这一页还没有作品'}</h4><p>{page === 1 ? '先给故事起个名字，再慢慢写下它的世界。' : '返回上一页，继续你的故事。'}</p></div>}
