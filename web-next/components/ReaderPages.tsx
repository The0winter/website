...
export default function ReaderPages(props:Props) {
  ...
  const commitPage=useCallback((target:number)=>{currentPage.current=target;setPage(target);},[]);
  const {viewport:windowRef,columns,surface,preview,begin:beginTurn,drag:dragTurn,finish:finishTurn,cancel:cancelTurn,busy:turnBusy,settling:turnSettling}=useReaderPageTurn({mode:turnMode,onCommit:commitPage});
  ...
  useLayoutEffect(()=>{
    ...
    const measure=()=>{
      ...
      const total=Math.max(1,scrolling?Math.ceil(body.scrollHeight/height):Math.ceil((body.scrollWidth+40)/(width+40)));
      ...
      } else if(previous.width && (previous.width!==width || previous.height!==height || previous.typography!==typography || previous.mode!==turnMode)) {
  ...
  },[paragraphs,counts,fontFamily,fontSize,lineHeight,paragraphGap,saveKey,scrolling,turnMode,chapter.id,cancelTurn,columns,windowRef,hydrated]);
  ...
  },[page,layout,counts,scrolling,columns,windowRef]);
  ...
  const turn=useCallback((direction:number)=>{
    ...
    if(target>=0 && target<layout.total){if(beginTurn(target,direction))finishTurn(true);return;}
    adjacentChapter(direction);
  },[controlsBlocked,props.navigating,layout.total,scrolling,turnBusy,onHideTools,adjacentChapter,beginTurn,finishTurn,windowRef]);
  useEffect(()=>{
  ...
  function pointerMove(event:React.PointerEvent) {
    ...
    if(target>=0 && target<layout.total){if(beginTurn(target,direction))dragTurn(distance);}
    else cancelTurn();
  }
  ...
  return <div className="reader-pages-root" data-dark={props.dark} data-mode={turnMode} data-reader-ready={layout.width>0} style={style}>
    <section className="reader-frame" data-paper={props.paper && !props.dark} aria-label="章节阅读">
      ...
      <div ref={windowRef} className="reader-page-window" tabIndex={0} aria-label={scrolling?'正文，可上下滚动，点击中央打开菜单':`${turnMode==='vertical'?'上下':'左右'}翻页，点击中央打开菜单`} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={cancelGesture} onLostPointerCapture={event=>{if(event.target===event.currentTarget)cancelGesture();}} onClick={click} onScroll={onScroll} onTouchStart={touchStart} onTouchEnd={touchEnd} onTouchCancel={()=>{touchEdge.current=null;}}>
        ...
        <div ref={columns} className="reader-columns" style={{fontFamily,fontSize:`${fontSize}px`,lineHeight,transform:scrolling?'none':`translateX(${-page*(layout.width+40)}px)`}}>
          {scrolling && props.previousId && <button className="reader-scroll-previous" onClick={()=>adjacentChapter(-1)}>上一章</button>}
          <h1>{title}</h1>
          ...
          <div className="reader-end"><p>{props.nextId?'本章完':'已读到最新章节'}</p><button disabled={props.navigating || !props.nextId} onClick={()=>adjacentChapter(1)}>{props.navigating?'加载中…':props.nextId?'下一章':'等待更新'}</button>{scrolling && props.nextId && <p className="reader-scroll-hint">继续上滑，阅读下一章</p>}</div>
        </div>
        </div>
