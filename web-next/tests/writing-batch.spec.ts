...
async function setup(page: Page, count = 3) {
  ...
  const state = {
    ...
    receipts: [] as string[], mutations: [] as {id: string; method: string; body?: WritingDraft}[], failId: '', loseResponseId: '',
  };
...
for (const width of [320, 390, 1440]) test(`compact chapter rows and animated management at ${width}`, async ({page}, info) => {
  ...
  expect(rowBox.height).toBeLessThanOrEqual(66);
...
test('batch deletion needs confirmation and failed cloud deletes remain visible', async ({page}) => {
  const state = await setup(page); await enter(page);
  await page.getByRole('button', {name: '批量管理', exact: true}).click(); await page.getByRole('button', {name: '全选', exact: true}).click();
  ...
  await remove.click(); await expect(page.getByRole('dialog')).toContainText('读者将无法再查看');
...
test('an empty chapter stops batch publication without losing it or later drafts', async ({page}) => {
  ...
  await enter(page); await page.getByRole('button', {name: '批量管理', exact: true}).click();
  await page.getByRole('button', {name: '全选', exact: true}).click();
