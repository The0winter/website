...
for (const width of [320,390,1440]) test(`creation, private list, unnamed publication and work management at ${width}`,async({page},info)=>{
  ...
  if(width===1440) expect((await page.locator('.work-create-form').boundingBox())!.width).toBeLessThanOrEqual(560);
...
for(const width of [320,390]) test(`mobile center, statistics and private draft resume at ${width}`,async({page},info)=>{
  ...
  expect(Math.abs(cover!.x+cover!.width/2-box!.x-box!.width/2)).toBeLessThan(1);
  expect(Math.abs(create!.x+create!.width/2-box!.x-box!.width/2)).toBeLessThan(1);
  expect(gear!.x+gear!.width).toBeLessThan(create!.x);
  await expect(card.getByRole('link',{name:'创作',exact:true})).toHaveCSS('font-size','20px');
