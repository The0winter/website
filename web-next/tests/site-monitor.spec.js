...
test('用户数据优先、正常状态收到底部、容量紧凑，日周月及缩放读数可用',async({page})=>{
  ...
  await expect(page.getByText('日活 DAU',{exact:true})).toBeVisible();await expect(page.getByText('周活 WAU',{exact:true})).toBeVisible();await expect(page.getByText('月活 MAU',{exact:true})).toBeVisible();
  await expect(page.getByText('周活 WAU',{exact:true}).locator('..').locator('.metric-value')).toContainText('91');
