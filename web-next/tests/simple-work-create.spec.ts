...
async function setup(page:Page) {
  const state={created:false,uploads:0,attempts:0,fail:false,keys:[] as string[],draft:{title,description:'风越过群山，将一封信送到海边。',cover_image:cover,category:'未分类',filename:'',chapters:[],revision:1}};
  ...
  await page.route('**/api/manuscripts/*',async route=>{
    ...
    expect(body.action).toBe('draft'); expect(body.chapters).toEqual([]); expect(body.revision).toBe(0);
    state.draft={...state.draft,...body,revision:1};
