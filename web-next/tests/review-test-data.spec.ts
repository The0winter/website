...
for(const width of [390,1440]) {
  test(`review preview and incremental sheet retain private profiles at ${width}px`,async({page},info)=>{
    ...
    await expect(sheet).toHaveCSS('height','720px');
    await expect.poll(async()=>Math.round((await sheet.boundingBox())?.y ?? -1)).toBe(180);
    ...
    await expect(sheet.getByText('已显示全部评论',{exact:true})).toBeVisible();
