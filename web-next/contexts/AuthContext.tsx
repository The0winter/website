...
export function AuthProvider({children}:{children:ReactNode}) {
 ...
 const accept=(user:AuthUser,profile:Profile)=>{setUser(user);setProfile(profile);localStorage.setItem('novelhub_user',user.id);};
 useEffect(()=>{
  let active=true;localStorage.removeItem('token');localStorage.removeItem('user');
  authApi.getSession().then(session=>{if(!active)return;if(session.user&&session.profile){setUser(session.user);setProfile(session.profile);localStorage.setItem('novelhub_user',session.user.id);}else localStorage.removeItem('novelhub_user');}).catch(()=>{if(active){setUser(null);setProfile(null);}}).finally(()=>{if(active)setLoading(false);});
  return()=>{active=false;};
 },[]);
