  ...
  return (
    ...
    <div className="min-h-screen bg-[#f4f5f7] py-2 md:py-6">
      <div className="max-w-[1200px] mx-auto px-2 md:px-4 flex flex-row gap-2 md:gap-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-2 md:p-4 sticky top-[25vh] md:top-4 z-10">
                        ...
                        return (
                            <div key={book.id} className="group flex p-3 md:p-5 gap-3 md:gap-5 hover:bg-gray-50 transition-colors relative items-center">
                                ...
                                <BookLink href={`/book/${book.id}`} className="relative flex-shrink-0 w-16 h-24 md:w-24 md:h-32 shadow-md rounded overflow-hidden group-hover:shadow-lg transition-all border border-gray-200">
                                     {book.cover_image ? (
                                         <BookCover priority={index<4} src={book.cover_image} alt={book.title || '小说封面'} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                                     ) : (
