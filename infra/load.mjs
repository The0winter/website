...
const source=JSON.parse(await fs.readFile('artifacts/staging-source.json','utf8'));
...
for(let i=0;i<seconds*rps;i++){
  ...
  const request=(async()=>{const begin=performance.now();const offsetMs=begin-started;try{const response=await fetch('http://127.0.0.1:8088'+paths[i%10],{signal:AbortSignal.timeout(15000)});const ttfb=performance.now()-begin;await response.arrayBuffer();samples.push({kind,offsetMs,status:response.status,ttfb,ms:performance.now()-begin});}catch{samples.push({kind,offsetMs,status:0,ms:performance.now()-begin,ttfb:15000});}})();
...
await fs.writeFile(`artifacts/load-${label}.json`,JSON.stringify({summary,source,samples},null,2));console.log(JSON.stringify(summary,null,2));
